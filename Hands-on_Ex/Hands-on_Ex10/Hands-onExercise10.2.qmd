...
```{r}
distPair <- distPair %>%
  dplyr:::rename.data.frame(orig = Var1,
         dest = Var2)
```
...
pop <- pop %>%
  left_join(mpsz,
            by = c("PA" = "PLN_AREA_N",
                   "SZ" = "SUBZONE_N")) %>%
  select(1:6) %>%
  dplyr:::rename.data.frame(SZ_NAME = SZ,
         SZ = SUBZONE_C)
```
...
flow_data1 <- flow_data1 %>%
  left_join(pop,
            by = c(ORIGIN_SZ = "SZ")) %>%
  dplyr:::rename.data.frame(ORIGIN_AGE7_12 = AGE7_12,
...
flow_data1 <- flow_data1 %>%
  left_join(pop,
            by = c(DESTIN_SZ = "SZ")) %>%
  dplyr:::rename.data.frame(DESTIN_AGE7_12 = AGE7_12,
...
The general formula of Unconstrained Spatial Interaction Model![](images/image1.jpg)
...
SIM_data <- SIM_data %>%
  cbind(df) %>%
  dplyr:::rename.data.frame(uncTRIPS = "uncSIM$fitted.values")
```
